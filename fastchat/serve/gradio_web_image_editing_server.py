...
def regenerate(state, request: gr.Request):
    ...
    logger.info(f"regenerate. ip: {ip}")
    # state.conv.update_last_message(None)
    return (state, gr.Image(height=512, width=512), "", "", gr.Image(height=512, width=512, type="pil"), "") + (disable_btn,) * 5
...
def clear_history(request: gr.Request):
    ...
    state = None
    return (state, None, "", "", None, "") + (disable_btn,) * 5
...
def add_text(state, model_selector, text_source, text_target, image_source, text_instruct, request: gr.Request):
    ...
    if len(text_source) <= 0 and len(text_target) <= 0:
        ...
        return (state, gr.Image(height=512, width=512), "", "", gr.Image(height=512, width=512, type="pil"), "") + (no_change_btn,) * 5
    ...
    return (state, gr.Image(height=512, width=512), text_source, text_target, image_source, text_instruct) + (disable_btn,) * 5
...
def build_about():
    about_markdown = about_markdown = f"""
# About Us
...
"""
    ...
    notice_markdown = f"""
# 🏔️ Play with Image Edition Models
{promotion}

## 🤖 Choose any model to edit image
"""
    state = gr.State()
